...
function cleanEnvironment() {
  garbageListStr
    .split(",")
    .forEach((key) => Object.defineProperty(window, key, { writable: false }));
}

function modifyPage() {
  // Block script, style and big images
  document
    .querySelectorAll("script, link[type], .main5down img")
    .forEach((el) => {
  ...
  if (isHomePage) {
    ...
    mainBoxEl.children[0].insertAdjacentHTML(
      "afterBegin",
      "<input id=tab-switch type=checkbox><label for=tab-switch>"
    );
  } else {
    forEachEl([".right", ".left", ".right-big"], (el) => mainBoxEl.append(el));
  }
  forEachEl([".menu", ".noticesbox", mainBoxEl], (el) =>
    document.body.append(el)
  );
  forEachEl([".container", ".main7"], (el) => el.remove());
  // Insert resources
  document
    .querySelector(".menu>ul>li:first-child>a")
    .insertAdjacentHTML("beforeEnd", `<img src="${iconData}">`);
  document.head.insertAdjacentHTML(
    ...
    <style>${cssStr}</style>
    <title>${
      "惠安一中 - " + (isHomePage ? "首页" : window.location.pathname)
    }</title>
  `
  );
}
...
function loadPagePlugin() {
  if (isHomePage) {
    ...
    let focuIndex = 0;
    setInterval(() => {
      focuContainer.style.setProperty(
        "--pic-index",
        focuIndex < focuNumber - 1 ? ++focuIndex : (focuIndex = 0)
      );
    }, 5000);
  } else {
    // Page number nav
    ...
    if (pageNumberContainer) {
      ...
      innerHTML +=
        currentPage === 1
          ? ""
          : `<a class=pgNext href="${pageName}1">首页</a><a class=pgNext href="${pageName}${
              currentPage - 1
            }">上一页</a>`;
      ...
      const endpager =
        currentPage === totalPage
          ? ""
          : `<a class=pgNext href="${pageName}${
              currentPage + 1
            }">下一页</a><a class=pgNext href="${pageName}${totalPage}">尾页</a>`;
